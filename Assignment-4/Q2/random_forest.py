...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for i in range(n):
            ...
            temp=XX[row][0:col-1]
            samples.append(XX[row][0:col-1])
            labels.append(XX[row][col-1])
    ...
    def fitting(self):
        ...
        for i,t in enumerate(self.decision_trees):
            t.learn(self.bootstraps_datasets[i],self.bootstraps_labels[i])
        pass      
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for t in self.decision_trees:
                    v.append(t.classify(record))

                y=np.append(y,np.argmax(np.bincount(v)))
